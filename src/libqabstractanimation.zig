...
pub const qabstractanimation = struct {
    ...
    ///
    /// ``` self: QtC.QAbstractAnimation, slot: fn (self: QtC.QObject, objectName: []const u8) callconv(.c) void ```
...
pub const qanimationdriver = struct {
    ...
    /// Wrapper to allow calling private signal
    ///
    /// ``` self: QtC.QAnimationDriver, slot: fn (self: QtC.QObject, objectName: []const u8) callconv(.c) void ```
    pub fn OnObjectNameChanged(self: ?*anyopaque, slot: fn (?*anyopaque, []const u8) callconv(.c) void) void {
        qtc.QObject_Connect_ObjectNameChanged(@ptrCast(self), @as(isize, @bitCast(@intFromPtr(&slot))));
    }
