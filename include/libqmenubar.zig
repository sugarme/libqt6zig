...
pub const qmenubar = struct {
    ...
    pub fn AddMenuWithTitle(self: ?*anyopaque, title: []const u8) QtC.QMenu {
        const title_str = qtc.struct_libqt_string{
            .len = title.len,
            .data = title.ptr,
        };
        return qtc.QMenuBar_AddMenuWithTitle(@ptrCast(self), title_str);
    }

    /// [Qt documentation](https://doc.qt.io/qt-6/qmenubar.html#addMenu)
    ///
    /// ``` self: QtC.QMenuBar, icon: QtC.QIcon, title: []const u8 ```
    pub fn AddMenu2(self: ?*anyopaque, icon: ?*anyopaque, title: []const u8) QtC.QMenu {
        const title_str = qtc.struct_libqt_string{
            .len = title.len,
            .data = title.ptr,
        };
        return qtc.QMenuBar_AddMenu2(@ptrCast(self), @ptrCast(icon), title_str);
    }
