...
pub const qscroller = struct {
    ...
    ///
    /// ``` target: QtC.QObject ```
    pub fn ScrollerWithTarget(target: ?*anyopaque) QtC.QScroller {
        return qtc.QScroller_ScrollerWithTarget(@ptrCast(target));
    }
    ...
    ///
    /// ``` self: QtC.QScroller, slot: fn (self: QtC.QObject, objectName: []const u8) callconv(.c) void ```
